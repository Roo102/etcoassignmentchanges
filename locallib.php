...
class assign {
    ...
    public function list_participants($currentgroup, $idsonly, $tablesort = false) {
        ...
        if (!isset($this->participants[$key])) {
            ...
            if ($instance->markingworkflow &&
                    ...
                    has_capability('mod/assign:completemarking', $this->get_context()) &&
    ...
    protected function view_grading_table() {
        ...
        if (get_config('assign', 'allowquickgrading')) {
    ...
    public function view_batch_markingallocation($mform) {
        ...
        list($sort, $params) = users_order_by_sql('u');
        // Only enrolled users could be assigned as potential markers.
        $markers = get_enrolled_users($this->get_context(), 'mod/assign:assessor', 0, 'u.*', $sort);
        $markerlist = array();
        foreach ($markers as $marker) {
    ...
    /**
     * Determine if this users grade can be edited by the grader (workflowstate on).
     *
     * @param int $userid - The student userid
     * @param bool $checkworkflow - whether to include a check for the workflow state.
     * @return bool $gradeentrydisabled
     */
    public function grade_entry_disabled($userid, $checkworkflow=true) {
        ...
        $gradeentrydisabled = $gradinginfo->items[0]->grades[$userid]->locked ||
                           $gradinginfo->items[0]->grades[$userid]->overridden;
        return $gradeentrydisabled;
    }
    ...
    public function get_marking_workflow_states_for_current_user() {
        ...
        $states = array();
        if (has_capability('mod/assign:grade', $this->context) && !has_capability('mod/assign:completemarking', $this->context)) {
